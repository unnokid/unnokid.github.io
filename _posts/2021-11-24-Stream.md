...
Java 8에서 추가된 Stream은 람다를 활용할 수 있는 기술 중 하나이다. 이전에는 배열 또는 Collection를 다루는 방법이 for 와 foreach 문을 돌면서 요소를 하나씩 꺼내서 다루는 방법을 사용했다. 하지만 복잡해질수록 코드의 양이 많아지고 로직이 섞이며 불편함을 겪었다. 이를 해결해줄 수있는게 스트림이다.
...
### 스트림 가공하기
 ...
 전체 요소중에서 여러가지 API를 이용해서 내가 원하는 것만 뽑아낼 수 있다.
...
<br/>
 ...
 >Predicate 는 boolean을 리턴하는 함수형 인터페이스이다.
  
 ```java
 //맨위 있던 예제 names 을 사용함
...
```
스트림에 들어가있는 값이 input이 되어 특정 로직을 지나 output이 되어 새로운 스트림에 담기게 되는 작업을 맵핑(mapping)이라고 한다.
...
//productList 에 각 product 개체의 수량을 꺼내기
//[23, 14, 13, 23 ,13]
