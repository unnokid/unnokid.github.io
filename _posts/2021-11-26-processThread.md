...
<br/>
<br/>
![프로세스 와 쓰레드](https://user-images.githubusercontent.com/58356031/143767384-479ac13d-e5ac-4a9e-bd34-0ef6f33b02e4.png)


## 프로세스 란
운영체제로부터 할당받은 작업의 단위이다. 안정성을 위해 프로세스마다 자신에게 할당된 메모리 내의 정보에만 접근 할 수 있도록 제약을 두었다.

> `프로그램`은 파일이 저장 장치에 저장되어 있지만 메모리에는 올라가 있지 않은 정적인 상태를 말한다.

## 쓰레드 란

프로세스내에서 실제로 작업을 수행하는 주체를 의미한다. 모든 프로세스에는 한 개 이상의 쓰레드가 존재하며 작업을 수행한다.
...
- 상호배제 - 한 리소스는 한번에 한프로세스만 사용할 수있다.
...
- (예방)방지
   - 상호 배제 조건을 제거  :
   - 점유와 대기 조건 제거 : 점유하지 않을때 다른 프로세스가 자원을 요구하도록 하는 방법 
   
   지원 과다 사용으로 인한 효율성 프로세스가 요구하는 자원을 파악하는데 대한 비용 자원에 대한 내용을 저장 및 복원하기 위한 비용 기아 상태 무한 대기등의 문제점이 있다.

   - 비선점 조건 제거 : 선점 가능한 프로토콜 생성
   - 환형 대기 : 자원 유형에 따라 순서를 매긴다.
...
- 탐지 및 회복
교착상태가 발생하면 발생 이후에 문제 해결
이를 해결할때 문제가 더큰경우 그냥 내둔다.
